...
template <class T>
string lexical_cast_hex( T t )
{
	ostringstream os;
	//os << "0x" << hex << setw(8) << setfill('0') << t << dec << "/*" << t << "*/";
	os << t << "/*0x" << hex << t << "*/";
	return os.str();
}

template <class T>
string lexical_cast_hex2( T t )
{
	ostringstream os;
	os << "// 0x" << hex << t;
	return os.str();
...
template <class T>
string lexical_cast_hex_addr( T t )
{
	ostringstream os;
	//os << "0x" << hex << setw(8) << setfill('0') << t << dec << "/*" << t << "*/";
	os << hex << t;// << dec << "/*" << t << "*/";
...
typedef pair<string, string> mn_intr_pair_t;
...
static const map<string, string> CustomIntrins( IntrinsicTemplateTable, IntrinsicTemplateTable + _countof(IntrinsicTemplateTable) );

/*
void gen_intrin_table()
{
	ifstream fin("gah.txt");
	ofstream fout("geh.txt");
	string line;
	while(getline(fin, line))
	{
		...
		string intrin		= line.substr(intrin_begin, intrin_end - intrin_begin);
		const string mnem	= line.substr(mnem_begin, mnem_end - mnem_begin);
		...
		fout << "mn_intr_pair_t(\"" << mnem << "\", " << "\"" << intrin << ";\")," << endl;
	}
...
static const string IntrinsicTemplates[] =
{
...
//static const regex RXMnem("\\$MNEM\\$");

static string ErrorUnknownInstruction( uint32_t Instruction )
{
	ostringstream oss;
...
static string GetIntrinsicForm( const string& Mnemonic, SPU_OP_TYPE itype )
{
	string result;
...
//static const regex rxRT("\\$RT\\$");

string spu_make_pseudo( const spu_insn* insn )
{	
	const string mnem = spu_decode_op_mnemonic(insn->raw);

	if ( mnem.empty() )
		return ErrorUnknownInstruction(insn->raw);

	const SPU_OP_TYPE itype = spu_decode_op_type(insn->raw);

	string result = GetIntrinsicForm( mnem, itype );

	const SPU_OP_COMPONENTS& OPComponents = insn->comps;

	const size_t LSLR_MASK = 0x3FFF0;
		
	replace_all( result, "$RT$", gpr_name(OPComponents.RT) ); 

	//result = regex_replace(result, rxRT, gpr_name(OPComponents.RT) );

	switch (itype)
	{
	case SPU_OP_TYPE_RRR:
		{				
			replace_all( result, "$RC$", gpr_name(OPComponents.RC) ); 
		}
	case SPU_OP_TYPE_RR:
		{
			replace_all( result, "$RB$", gpr_name(OPComponents.RB) ); 
			replace_all( result, "$CA$", (OPComponents.RA < 31) ? 
				ChannelNames[OPComponents.RA] 
				: lexical_cast((uint16_t)OPComponents.RA) );
		}
	...
	case SPU_OP_TYPE_RI16:
		{
			replace_all( result, "$RA$", gpr_name(OPComponents.RA) );				
			replace_all( result, "$LABEL$", lexical_cast_hex_addr( insn->vaddr + ((int32_t)OPComponents.IMM << 2) ) );

			//replace_all( result, "$QOFF$",		lexical_cast_hex( LSLR_MASK & ((int32_t)OPComponents.IMM << 4) ) );
			replace_all( result, "$WADDR_REL$", lexical_cast_hex( LSLR_MASK & (insn->vaddr + ((int32_t)OPComponents.IMM << 2)) ) );
			replace_all( result, "$WADDR_ABS$", lexical_cast_hex( LSLR_MASK & ((int32_t)OPComponents.IMM << 2) ) );

			ostringstream oss;
			oss << "sub_" << hex << (insn->vaddr + ((int32_t)OPComponents.IMM << 2)) << "();";
			...
			if ( "brnz" == mnem || "brz" == mnem || "brhnz" == mnem || "brhz" == mnem )
			{
				...
				{
					result = string("IF") + result;
				}
				else
				{
					result = string("WHILE") + result;
				}
			}
			else if ( "stop" == mnem )
			{
