...
static __m128* __ls_128 = nullptr;
...
#define CALL_I7(type, handler)		case spu_op::type: RT = handler(RA, (int16_t)IMM); break;
#define CALL_I8(type, handler)		case spu_op::type: RT = handler(RA, (int16_t)IMM); break;
#define CALL_I10(type, handler)		case spu_op::type: RT = handler(RA, (int16_t)IMM); break;
#define CALL_I16(type, handler)		case spu_op::type: RT = handler((int16_t)IMM); break;
...
void spu_vm_dostep(spu_vm* vm)
{
	...
	int32_t IMM = (int32_t)PC->comps.IMM;
	...
	switch (PC->op)
	{
		...
		case spu_op::M_IOHL: RT = si_iohl(RT, (int16_t)IMM); break;
